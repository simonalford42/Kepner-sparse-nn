...
function kronecker_emr_netowrk(Ns::Array{Array{Int, 1}, 1}, B::Array{Int, 1}, D::Union{Array{Int, 1}, Void}=nothing)
	M = sum(length.(Ns))
	...
	preW = extended_mixed_radix_newtork(Ns)

        W = Array{SparseMatrixCSC{Int32, Int32}, 1}()
        for j in 1:length(W)
		gd = div(gs[j], D[j])
		Gamma = zeros(Int32, (g[j], g[j]))
		for k in 0:(d-1)
